...
global_varideclare: VAR CL var_list SM;
var_list: var_item (CM var_item)*;
var_item: (ID | (ID (LSB INTEGER RSB)+)) ('=' initial_value)?;
...
assign_statement: lhs '=' expression SM;
...
signn: signn (LSB | RSB) | endofex;
endofex: LP expression RP | para_item | call_statement;

// Left hand side define
lhs: ID | array_cell;
array_cell: expression (LSB expression RSB)*;

// If statement
if_statement: if_then_stmt else_if_stmt* else_stmt? ENDIF DOT;
if_then_stmt: IF expression THEN global_varideclare* statement*;
else_if_stmt: ELSEIF expression THEN global_varideclare* statement*;
else_stmt: ELSE global_varideclare* statement*;

// For statement
for_statement: FOR LP ID '=' expression CM expression CM expression RP DO
                global_varideclare* statement*
                ENDFOR DOT;

// While statement
while_statement: WHILE expression DO global_varideclare* statement* ENDWHILE DOT;

// Do-while statement
dowhile_statement: DO global_varideclare* statement* WHILE expression ENDDO DOT;
